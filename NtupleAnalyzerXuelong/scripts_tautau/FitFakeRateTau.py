...
def fit_histo(cut, decaymode, variablename , variabletitle,leadingname,leading_str):
    ...
    if (year=="2018"):
        if leadingname =="1":
            ...
            if decaymode=='11':
                fitfunc.SetParameter(0,0.13)
                fitfunc.SetParameter(1, -357)
                fitfunc.SetParameter(2, -168)
                fitfunc.SetParameter(3, 2.78)
        else:
    ...
    if (year=="2017"):
        if leadingname =="1":
            ...
            if decaymode=='11':
                ...
                fitfunc.SetParameter(1, -10.3)
                #fitfunc.SetParLimits(1,-0.02,0)
                fitfunc.SetParameter(2, -1779)
                fitfunc.SetParameter(3, 262)
        else:
            fitfunc.SetParameter(0, 1.8)
            fitfunc.SetParameter(1, -9)
            fitfunc.SetParameter(2, 130)
...
def fit_histo_ntrk(cut, decaymode, variablename , variabletitle,leadingname,leading_str):
    ...
    TGraph_FR.SetName("FRhisto_{}_dm{}_{}_tauh{}".format(cut, decaymode, variablename,leadingname))
    TGraph_FR.Write()
    return fitfunc1,fitfunc2,FR_ntrk0,FRe_ntrk0,FR_ntrk1,FRe_ntrk1 
...
for leadingname in ["1","2"]:
    ...
    for decaymode in [0,1,10,11]:
        for variable in variablelist:
            ...
            if (variable==ntrk):
                fitfunc1,fitfunc2,FR_ntrk0,FRe_ntrk0,FR_ntrk1,FRe_ntrk1 = fit_histo_ntrk(cut,decaymode,variable.name,variable.title,leadingname,leading_str)
