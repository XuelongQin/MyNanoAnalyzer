...
if sample == "DY":
    ...
    fout.Close()
    exit(0)'''
    histo_tt2 = DY_rescale(histoDYhigh,histoDY_tt2,0.194)
    
    print ("tt_2 basic ", histo_tt2.Integral())
    
    dir2.cd()
    histo_tt2.SetName("{}".format(name))
    histo_tt2.Write()
    ...
    for i in range(len(uncertainty)):
        ...
        df_tt2_sys = df_sys(df,tt_2cut+isocut, sysflag, uncertainty_func[i])
        histoDYhigh_sys = gethisto(df_DYhigh_sys,"DYhigh_{}".format(uncertainty_name), nbins, binning)
        histoDY_tt2_sys = gethisto(df_tt2_sys,"tt2_{}".format(uncertainty_name), nbins, binning)
        histo_tt2_sys = DY_rescale(histoDYhigh_sys,histoDY_tt2_sys,0.194)
        print ("tt_2 ", uncertainty_name, " ", histo_tt2_sys.Integral())
        dir2.cd()
        histo_tt2_sys.SetName("{}{}".format(name,uncertainty_name))
        histo_tt2_sys.Write()


else:
    df_tt2 = df.Filter(tt_2cut+isocut)
    histo_tt2 = gethisto(df_tt2,"tt2", nbins, binning)
    print ("tt_2 basic ", histo_tt2.Integral())
    dir2.cd()
    histo_tt2.SetName("{}".format(name))
    histo_tt2.Write()
    ...
    if (name != "data_obs"):
        for i in range(len(uncertainty)):
            ...
            if ("taues" in uncertainty_name):
                ...
                sysflag = 0
            df_tt2_sys = df_sys(df,tt_2cut+isocut, sysflag, uncertainty_func[i])
            histo_tt2_sys = gethisto(df_tt2_sys,"tt2_{}".format(uncertainty_name), nbins, binning)
            print ("tt_2 ", uncertainty_name, " ", histo_tt2_sys.Integral())
            dir2.cd()
            histo_tt2_sys.SetName("{}{}".format(name,uncertainty_name))
            histo_tt2_sys.Write()
