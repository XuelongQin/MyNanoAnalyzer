...
ROOT::RVec<int> Getmutauindex(int nLepCand, ROOT::VecOps::RVec<int> &LepCand_id ,ROOT::VecOps::RVec<float> &LepCand_dz){
    ...
    if (nLepCand==2){
        ...
        ROOT::VecOps::RVec<float> abs_diff_mudz = abs(LepCand_dz-Min(LepCand_dz));
...
float Getxsweight_W(float LHE_Njets){
    ...
    else if (LHE_Njets==1) weight=luminosity*nnlo/(LW+LW2);
    else if (LHE_Njets==1) weight=luminosity*nnlo/(LW+LW3);
    else if (LHE_Njets==1) weight=luminosity*nnlo/(LW+LW4);
    return weight;
}
