...
def gethisto_anti(df,name,var, nbins,binning):
    hmodel = TH1DModel("h_{}_{}".format(var,name),"",nbins,binning)
    histo = df.Redefine("mvis","TMath::Min(float(199.),float(mvis))")\
        .Define("plotweight","totalweight*FR").Histo1D(hmodel,var,"plotweight").GetPtr()
    return histo
...
def gethisto(df,name,var,nbins,binning):
    hmodel = TH1DModel("h_{}_{}".format(var,name),"",nbins,binning)
    histo = df.Redefine("mvis","TMath::Min(float(199.),float(mvis))")\
        .Define("plotweight","totalweight").Histo1D(hmodel,var,"plotweight").GetPtr()
    return histo

def gethisto_BSM(df,name,var,TauG2Weights,nbins,binning):
    hmodel = TH1DModel("h_{}_{}_{}".format(var,name,TauG2Weights),"",nbins,binning)
    histo = df.Redefine("mvis","TMath::Min(float(199.),float(mvis))")\
        .Define("plotweight","totalweight*{}".format(TauG2Weights)).Histo1D(hmodel,var,"plotweight").GetPtr()
    return histo
