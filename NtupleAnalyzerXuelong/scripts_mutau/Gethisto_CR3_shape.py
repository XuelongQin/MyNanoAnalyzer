...
'''if sample == "DY":
    ...
    df_mt3 = df.Filter(mt_3cut+isocut)
    
    histoDYhigh = gethisto(df_DYhigh,"DYhigh","Acopl", nbins, binning)
    histoDY_mt3 = gethisto(df_mt3,"mt3", "Acopl",nbins, binning)

    histo_mt3 = DY_rescale(histoDYhigh,histoDY_mt3,0.0501+0.0242)

    
    print ("mt_3 basic ", histo_mt3.Integral())
    dir3.cd()
    histo_mt3.SetName("{}".format(name))
    histo_mt3.Write()
    ### now systematic part
    for i in range(len(uncertainty)):
        ...
        df_mt3_sys = df_sys(df,mt_3cut+isocut, sysflag, uncertainty_func[i])
        histoDYhigh_sys = gethisto(df_DYhigh_sys,"DYhigh_{}".format(uncertainty_name),"Acopl", nbins, binning)
        histoDY_mt3_sys = gethisto(df_mt3_sys,"mt3_{}".format(uncertainty_name),"Acopl", nbins, binning)
        histo_mt3_sys = DY_rescale(histoDYhigh_sys,histoDY_mt3_sys,0.0501+0.0242)
        print ("mt_3 ", uncertainty_name, " ", uncertainty_func[i]," ", histo_mt3_sys.Integral())
        dir3.cd()
        histo_mt3_sys.SetName("{}{}".format(name,uncertainty_name))
        histo_mt3_sys.Write()

else:
'''
df_mt3 = df.Filter(mt_3cut+isocut)
histo_mt3 = gethisto(df_mt3,"mt3","Acopl", nbins, binning)
print ("mt_3 basic ", histo_mt3.Integral())
dir3.cd()
histo_mt3.SetName("{}".format(name))
histo_mt3.Write()
...
if (name != "data_obs"):
    for i in range(len(uncertainty)):
        ...
        if ("taues" in uncertainty_name):
            ...
            sysflag = 0
        df_mt3_sys = df_sys(df,mt_3cut+isocut, sysflag, uncertainty_func[i])
        histo_mt3_sys = gethisto(df_mt3_sys,"mt3_{}".format(uncertainty_name),"Acopl", nbins, binning)
        print ("mt_3 ", uncertainty_name, " ", uncertainty_func[i]," ", histo_mt3_sys.Integral())
        dir3.cd()
        histo_mt3_sys.SetName("{}{}".format(name,uncertainty_name))
        histo_mt3_sys.Write()
