...
def buildCondorFile(opt,FarmDirectory):
    ...
    with open (condorFile,'w') as condor:
        ...
        for dataset in datasets:
          ...
          if 'SingleMuon' in dataset.split('/') or 'Tau' in dataset.split('/') or 'EGamma' in dataset.split('/') or 'MuonEG' in dataset.split('/') or 'DoubleMuon' in dataset.split('/'):
            sufix='data'
            dataset_name = dataset.split('/')[10]
            file_list=glob.glob(dataset+'/*.root')
            print (file_list)
          elif 'eos' in dataset.split('/'):
            sufix='mc'
            if 'GGToTauTau_Inclusive_M_50GeV_PTFilter' in dataset.split('/'):
              dataset_name = dataset.split('/')[9]
            else:
              dataset_name = dataset.split('/')[8]
            file_list=glob.glob(dataset+'/*.root')
            print (file_list)
    ...
    with open(workerFile,'w') as worker:
        ...
        worker.write('echo "--bo $CMSSW_BASE/src/MyNanoAnalyzer/TauG2/scripts/keep_out_mutau.txt  \\\\"\n')
        ...
        worker.write('--bo $CMSSW_BASE/src/MyNanoAnalyzer/TauG2/scripts/keep_out_mutau.txt  \\\n')
...
def main():
    ...
    parser.add_option('-i', '--in',     dest='input',  help='list of input datasets',    default='/afs/cern.ch/user/x/xuqin/work/taug-2/nanoAOD/CMSSW_10_6_27/src/MyNanoAnalyzer/TauG2/data/ListSamplesMCrenanoaod2018_W.txt', type='string')
    parser.add_option('-o', '--out',      dest='output',   help='output directory',  default='/eos/cms/store/cmst3/group/taug2/AnalysisXuelong/ntuple_after_analyzer/mutau', type='string') #EDIT THIS
